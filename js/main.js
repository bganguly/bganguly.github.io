    ...
    const _spinnerSvg = `<svg style="width:12px;height:12px;flex-shrink:0;animation:_lbspin 0.9s linear infinite" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><style>@keyframes _lbspin{to{transform:rotate(360deg)}}</style><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" style="opacity:.25"/><path fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" style="opacity:.8"/></svg>`;
    function _liveBtnLoad(el) {
      const orig = el.innerHTML;
      const origStyle = el.getAttribute('style') || '';
      el.innerHTML = _spinnerSvg + '&nbsp;Backend waking up…';
    ...
    function liveBtn(c, col, panelId, localMode) {
      ...
      if (effectiveLiveUrl && effectiveLiveUrl !== '#') {
        ...
        const _coldClick = c.coldStart ? ' onclick="_liveBtnLoad(this)"' : '';
        return `<a href="${effectiveLiveUrl}" ${_tgt}${_coldClick} class="${cls}" style="background:${col.btn};border:1px solid ${col.btnBorder};color:${col.btnText}">${arrowIcon}${c.liveLabel}</a>`;
      }
